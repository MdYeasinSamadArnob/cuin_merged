...
-- on every candidate pair of the 5k fixture. This is what makes it
safe to replace the pipeline's per-pair Python loop
(pipeline.duckdb_orchestrator._stage_score_and_decide) with the
compiled SQL for the instant-redecide path: if this test passes, the
two approaches always agree.
"""
...
def test_sql_decision_matches_python_oracle_on_every_pair():
    ...
    sql = compile_decision_sql(DEFAULT_SCORING_RULES, DuckDbDialect())
    con.execute(sql)
    sql_rows = con.execute("SELECT a_key, b_key, decision FROM pair_decisions").fetchall()
...
def test_decision_distribution_is_not_trivially_all_reject():
    ...
    sql = compile_decision_sql(DEFAULT_SCORING_RULES, DuckDbDialect())
    con.execute(sql)
    rows = con.execute("SELECT decision, COUNT(*) FROM pair_decisions GROUP BY decision").fetchall()
